...
Use `org.freedesktop.Platform` 23.08 runtime and bundle PyQt6 with `pip install`.

### systemd user service (auto-start on login)

```ini
# ~/.config/systemd/user/timehud.service
[Unit]
Description=TimeHUD HUD

[Service]
ExecStart=/path/to/timehud/timehud
Restart=on-failure
...
```bash
systemctl --user enable --now timehud
```
...
|---------|---------|-----------|
| PyQt6 ≥ 6.4 | GUI, rendering, timers | ✅ Yes |
| pynput ≥ 1.7 | Global hotkeys | Optional |
| paplay / aplay / ffplay / mpv | Audio playback | Optional (for sound) |
...
timehud/
├── src/timehud/
│   ├── main.py             # Entry point, argparse, hotkeys
│   ├── __main__.py         # Package entry
│   ├── overlay.py          # Main HUD window (with arc, tray, etc)
│   ├── config.py           # Persistent settings dataclass
│   ├── sound_manager.py    # WAV generation + playback
│   └── settings_dialog.py  # Qt settings dialog
├── requirements.txt
├── install.sh
├── timehud                 # Launcher script
└── timehud.desktop
```
