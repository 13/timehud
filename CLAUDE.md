...
- **`timer_engine.py`** — `TimerEngine`, the Qt-free timer state machine (stopwatch/countdown, pause/resume, beep scheduling). The UI calls `tick()` every 100 ms and gets a `TickResult(display, state, beeps, finished, restarted)`; state is `"run" | "pause" | "warn" | "end"`. Injectable `clock` parameter makes tests deterministic. **Keep this module free of Qt imports** — the test suite depends on it running headless.
