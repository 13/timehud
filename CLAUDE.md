...
- Beep timing: last-5 shorts at displayed 5..1, long beep exactly at 00:00 (countdown finish, every interval/cycle phase end, session end — gated by `phase_beeps`). The fast double beep is exclusively the optional `halfway_beep` cue at half of each work phase. Cycle modes never play periodic `sound_interval` beeps or the `sound_alert_before` pre-beep. Pinned by tests.
